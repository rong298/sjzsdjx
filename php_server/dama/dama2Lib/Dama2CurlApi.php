...
class Dama2Api{
    ...
    private function set_auth($auth){
        ...
        $redis->set($this->prefix_sess . 'auth', urldecode($auth));
        $redis->set($this->prefix_sess . 'time', time());
    }
    ...
    private function is_auth_alive(){
        ...
        $un = $redis->get($this->prefix_sess . 'name');
        $passwd = $redis->get($this->prefix_sess . 'password');

        if(!isset($auth) || time() - $time > $this->expire_time ||
            $un !== $this->username || $passwd !== $this->password){
            return false;
        }else{
            return true;
        }
